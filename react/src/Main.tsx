...
const questionsData = [
  ...
  },
  {
    ...
    answers: [
      '여행 일정표처럼 상세하게 계획된 스케줄이 좋아',
      '그때그때의 기분과 상황에 따라 결정하는 게 좋아',
    ],
  },
  {
    category: 'J/P',
...
const ProgressBar: React.FC<{ current: number; total: number }> = ({
  ...
  return (
    <div className="relative h-3 bg-gray-200 rounded-full mt-10 lg:mb-20 xs:mb-10 w-[80%]">
      <div
        style={{ width: `${width}%` }}
        className="h-full bg-custom-pink rounded-full z-10 absolute top-half transform -translate-y-half"
      ></div>
      ...
      {Array(total)
        ...
        .map((_, i) => (
          <div
            ...
            className={`absolute top-[15%] transform -translate-x-half w-[9.5px] h-[9.5px] border-white border-solid border-opacity-[0.8] border-[4px] rounded-full z-20 transition-all duration-[500ms] ${
              current > i ? 'scale-[1.3]' : ''
            }`}
          >
            {current > i && (
          ...
          {questionsData[currentQuestionIndex].answers.map((answerText, i) => (
            <div key={i} className="mb-2">
              <button
                ...
                className="w-full px-6 h-16 text-white bg-custom-black2 rounded-xl top-300 font-custom2"
              >
                {' '}
                {answerText}{' '}
              </button>{' '}
